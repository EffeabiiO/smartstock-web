...
type ActiveTask = AppProductionTask & { product: AppProduct | null }
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const activeOrders = orders.filter(
        (o) => !['completato', 'annullato'].includes(o.status)
      )
      const lowStock = products.filter(
        (p) => p.available_quantity <= p.minimum_stock && p.minimum_stock > 0
      )
      ...
      const revenue = typedMonthOrders.reduce(
        (sum, order) => sum + (order.total_revenue ?? 0),
        0
      )
      ...
      setStats({
        ...
        active_production: tasks.length,
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        <div className="card p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/ordini" className="text-sm text-blue-600 hover:underline">
              Vedi tutti
            </Link>
          </div>
            ...
            <div className="space-y-2">
              {recentOrders.map((o) => (
                <Link
                  key={o.id}
                  href={`/ordini/${o.id}`}
                  className="flex items-center justify-between p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-gray-800 transition-colors"
                >
                  <div>
                  ...
                  <div className="text-right">
                    <span className={`status-badge ${getOrderStatusColor(o.status)}`}>
        ...
        <div className="card p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/produzione" className="text-sm text-blue-600 hover:underline">
              Vedi tutti
            </Link>
          </div>
          {activeTasks.length === 0 ? (
            ...
            <div className="space-y-2">
              {activeTasks.map((t) => (
                <div
                  key={t.id}
                  className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-gray-800"
                >
                  <div>
                    <p className="text-sm font-medium text-slate-900 dark:text-slate-100">
                      {t.product?.name ?? '—'}
                  ...
                  <span className={`status-badge ${getProductionStatusColor(t.status)}`}>
                    {getProductionStatusLabel(t.status)}
                  </span>
                </div>
              ))}
            </div>
      ...
      {lowStockProducts.length > 0 && (
        <div className="card p-5 border-l-4 border-amber-400">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="section-title text-amber-700 dark:text-amber-400">
              Alert Scorte Basse
            </h2>
          </div>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {lowStockProducts.map((p) => (
              <Link
                ...
                className="p-3 bg-amber-50 dark:bg-amber-950 rounded-xl hover:bg-amber-100 dark:hover:bg-amber-900 transition-colors"
              >
                <p className="text-sm font-semibold text-slate-900 dark:text-slate-100 truncate">
                  {p.name}
                </p>
                <p className="text-xs text-amber-700 dark:text-amber-400 mt-1">
                  Disp: {p.available_quantity} / Min: {p.minimum_stock}
                </p>
              </Link>
            ))}
          </div>
